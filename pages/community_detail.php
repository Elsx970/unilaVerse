...
<head>
    ...
    <title><?= htmlspecialchars($community['name']) ?> - UnilaVerse</title>
...
<section class="max-w-4xl mx-auto mt-8 bg-white p-6 rounded shadow">
    <h2 class="text-2xl font-semibold text-blue-700 mb-2"><?= htmlspecialchars($community['name']) ?></h2>
    <p class="text-gray-600 mb-4"><?= htmlspecialchars($community['description']) ?></p>
    <a href="create_post.php?community_id=<?= $community_id ?>" class="bg-blue-600 text-white px-4 py-2 rounded hover:bg-blue-700 inline-block"> ➕ Buat Postingan</a>
</section>

<form method="GET" class="max-w-4xl mx-auto mt-6 flex">
    <input type="hidden" name="id" value="<?= $community_id ?>">
    <input type="text" name="q" placeholder="Cari di komunitas..."
...
<?php
// Filter post jika ada query pencarian
$search = isset($_GET['q']) ? $conn->real_escape_string($_GET['q']) : '';
$whereClause = $search ? "AND p.content LIKE '%$search%'" : '';
...
$posts = $conn->query("
    ...
    FROM posts p
    JOIN users u ON p.user_id = u.id
    WHERE p.community_id = $community_id $whereClause
